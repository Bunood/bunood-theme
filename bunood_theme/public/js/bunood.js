...
(function () {
	...
	function docbar_wanted(frm) {
		...
		return !wrapper?.classList.contains("bnd-bill-simple-active") &&
	...
	function mount_docfoot(frm) {
		...
		const wanted = docfoot_wanted() && docbar_wanted(frm) && !page.querySelector(".bnd-form-actionbar") && !!(frm.page.btn_primary && frm.page.btn_primary.length);
		if (!wanted) {
			if (foot) foot.remove();
