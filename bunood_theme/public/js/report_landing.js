...
(() => {
	...
	const REPORTS = [
		{ label: "Finance and close", description: "Review period readiness, exceptions and close evidence.", icon: "calendar", route: ["bnd-finance-close"] },
		{ label: "Journal workbench", description: "Prepare and review native journal entries with their supporting details.", icon: "book-open", route: ["bnd-journal-workbench"] },
		{ label: "Bank Reconciliation", description: "Match bank activity to native transactions and investigate differences.", icon: "banknote", route: ["bnd-banking"] },
	...
	function mount() {
		...
		const cards = REPORTS.map(cardFor);
		grid.append(...cards);
