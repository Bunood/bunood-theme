...
class PaymentSetupContractTests(unittest.TestCase):
    ...
    def test_all_lifecycles_and_existing_site_patch_run_setup(self):
        ...
        # GitHub main has install and migrate hooks; its setup wizard is not hooked.
        # Integration v0.48.0: held -- present once, in ensure_pos_retail only.
        self.assertEqual(setup.count("ensure_pos_payment_setup()"), 1)
        self.assertIn("ensure_pos_payment_setup()", setup.split("def ensure_pos_retail")[1])
        # Integration v0.48.0: the data patches are listed as HELD, not registered.
        for patch in ("v0_46_11.pos_payment_ledgers", "v0_46_33.network_mode_english"):
            self.assertIn(patch, patches)
            self.assertNotRegex(patches, r"(?m)^bunood_theme\.patches\." + re.escape(patch))
