...
test('native form action bar is sticky and stands down for Simple workbenches', () => {
  ...
  // Integration v0.48.0: the bar is opt-in through form_foot = "Action Bar";
  // the official Pinned Bar gate is unchanged apart from yielding to a mounted bar.
  assert.match(desk, /"Action Bar": "actions"/);
  assert.match(desk, /getAttribute\("data-bnd-form-foot"\) === "actions"/);
