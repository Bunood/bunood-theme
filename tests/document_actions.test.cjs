...
test('form action bars are sticky and do not stack with the former bottom proxy', () => {
  ...
  const billCss = fs.readFileSync('bunood_theme/public/scss/surfaces/_sales_bill.scss', 'utf8');
  ...
  assert.match(desk, /docfoot_wanted\(\) && !page\.querySelector\("\.bnd-form-actionbar"\)/);
  ...
  assert.match(simple, /this\.actions = create\("div", "bnd-simple-actions", null, null\)/);
  assert.match(billCss, /\.bnd-simple-actions \{[\s\S]*?position: sticky/);
  assert.match(billCss, /\.bnd-bill-toolbar \{\s*position: sticky;/);
  ...
  for (const selector of ['bnd-simple-actions', 'bnd-bill-intro', 'bnd-bill-toolbar']) {
    assert.doesNotMatch(billCss, new RegExp(`data-bnd-topbar[^\\n]*\\.${selector}[^\\n]*inset-block-start: calc\\(var\\(--bnd-topbar-h\\) \\+ var\\(--page-head-height\\)\\)`));
  }
});
