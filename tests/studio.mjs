...
// The month the BOOKS live in, measured — the suite must never depend on
// today's calendar (it went red the first September after an August seeding).
const MONTH = { from: "", to: "" };
...
`
	...
	if (!m) throw new Error("could not mint the data month: " + out.slice(-200));
	MONTH.from = m[1];
	MONTH.to = m[2];
}
...
async function main() {
	...
	recover = async () => {
		try {
			// مودالات عالقة (msgprint أو حوار فترة) تحجب كل نقر — اطردها حتى تزول.
			for (let i = 0; i < 3 && (await page.$(".modal.show")); i++) {
	...
	const setDataPeriod = async () => {
		...
		await page.evaluate(async ([from, to]) => {
			// set_value غير متزامن في فرابي 16 — النقر قبل اكتماله يرسل القيم القديمة (مقيس).
