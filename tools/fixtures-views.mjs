...
const PY = [
	...
	'    for name in frappe.get_all("File", filters={"attached_to_doctype": "Item", "attached_to_name": ["like", "BND-VIEW-%"]}, pluck="name"):',
	...
	"# The image view needs a File URL in Item.image. Data URIs are intentionally",
	"# rejected by current Frappe sanitisation, so use a tiny deterministic PNG",
	"# attachment instead. One item remains imageless for the no-image branch.",
	'PNG = base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M/wHwAF/gL+Avz8WQAAAABJRU5ErkJggg==")',
	"",
	...
	"    doc.insert(ignore_permissions=True, ignore_mandatory=True)",
	"    if i != 11:",
	'        file_doc = save_file(f"{code}.png", PNG, "Item", code, is_private=0)',
	'        frappe.db.set_value("Item", code, "image", file_doc.file_url, update_modified=False)',
