...
for (const key of items) {
	...
	const opts = await page.evaluate(() => {
		...
		const CRUMBS_ONLY = ".bnd-cbp-style:not(.bnd-lp-card):not(.bnd-plp-style):not(.bnd-ibp-style):not(.bnd-stp-style):not(.bnd-lvp-style):not(.bnd-fvp-style):not(.bnd-wsp-style):not(.bnd-chp-style):not(.bnd-icp-style)";
		const IMPLICIT = {
