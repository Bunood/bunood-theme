/**
 * Structural fingerprint of every picker: the sequence of tag.class plus the
 * count of non-empty text nodes and SVGs. Counting nodes rather than characters
 * keeps the fixture valid in both Arabic and English while still catching a dropped
...
const SITE=process.env.BND_SITE||"demo.bunood.test", BACKEND=process.env.BND_BACKEND||"bunood-backend-1", URL_BASE=process.env.BND_URL||"http://localhost:8080";
...
const fp = await page.evaluate(()=>{
  ...
  for (const f of ["theme_picker","layout_picker","sidebar_picker","crumbs_picker","palette_picker","inbox_picker","user_picker","links_picker","search_picker","status_picker",
    ...
    let texts=0;
    while(textWalker.nextNode()) if(textWalker.currentNode.nodeValue.trim()) texts++;
...
for (const [k,v] of Object.entries(fp)) console.log(k.padEnd(16), v?`nodes=${v.n} svg=${v.svgs} texts=${v.texts}`:"ABSENT");
await b.close();
